"""LN-only TTA variant of TTA_main_csv.py.

Same loop as TTA_main_csv.py, but uses tableshift.PFT3A_src.PFT3A_ln (which
only updates LayerNorm/BN affine params). Outputs land at
``checkpoint_tta_ln.pt`` and ``metrics_tta_ln.json`` so the original TTA
...
from tableshift.PFT3A_src.PFT3A_ln import PFT3A
...
def main(experiment, data_dir, model, ckpt_root, n_epochs, batch_size, debug):
    ...
    banner = (f" TTA-LN | experiment={experiment} | model={model} | "
    ...
    with torch.enable_grad():
        for epoch in range(1, n_epochs + 1):
            ...
            for batch in tqdm(ood_loader,
                              desc=f"{experiment}:pft3a_ln:tta-ep{epoch}"):
            ...
            print(f"[tta-ln] epoch {epoch}: updated on {ep_used}/{ep_seen} "
    ...
    print(f"[tta-ln] ood_test acc={acc:.4f} "
          f"balanced_acc={bal_acc:.4f} f1={f1:.4f}", flush=True)
    print(f"[tta-ln] total updates on {total_used}/{total_seen} OOD samples "
    ...
    dst_ckpt = os.path.join(out_dir, "checkpoint_tta_ln.pt")
    torch.save(
        (pft3a.base_model1.state_dict(),
         pft3a.optimizer_list[0].state_dict()),
        dst_ckpt)
    print(f"[save] tta-ln checkpoint -> {dst_ckpt}", flush=True)
    ...
    metrics_path = os.path.join(out_dir, "metrics_tta_ln.json")
    with open(metrics_path, "w") as f:
        json.dump(metrics_payload, f, indent=2)
    print(f"[save] tta-ln metrics    -> {metrics_path}", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--experiment", default="acsunemployment",
                        choices=SUPPORTED_EXPERIMENTS)
    parser.add_argument("--model", default="ft_transformer")

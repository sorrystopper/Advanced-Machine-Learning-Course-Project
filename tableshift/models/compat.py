...
class SklearnStylePytorchModel(ABC, nn.Module):
    ...
    def evaluate(self, eval_loaders: Dict[str, DataLoader], device):
        exp = getattr(self, "config", {}).get("exp", None)
        return {str(split): evaluate(self, loader, device, split, exp=exp)
    ...
    def fit(self, train_loaders: Dict[Any, DataLoader],
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            train_dur = time.perf_counter() - epoch_t0
            ...
            print(f"[time] epoch {epoch}/{n_epochs}: "
                  f"train {train_dur:.2f}s, eval {epoch_dur - train_dur:.2f}s, "
                  f"total {epoch_dur:.2f}s", flush=True)
        ...
        total = time.perf_counter() - fit_t0
        avg = total / max(n_epochs, 1)
        print(f"[time] training finished: total {total:.2f}s "
              f"({n_epochs} epochs, avg {avg:.2f}s/epoch)", flush=True)
        return fit_metrics
